...
/*
Create another struct named feedbackResult and assign it four fields:
feedbackTotal, feedbackPositive, feedbackNegative, and feedbackNeutral. All field type are int.
*/
...
func generateRating() {
	...
	for _, v := range f.Models {
		...
		for _, msg := range v.Feedback {
			if text := strings.Split(msg, " "); len(test) >= 5 {
				vehRating = 5.0
